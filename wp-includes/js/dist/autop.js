...
Object.defineProperty(exports,'__esModule',{value:true});};}();var __webpack_exports__={};__webpack_require__.r(__webpack_exports__);__webpack_require__.d(__webpack_exports__,{"autop":function(){return autop;},"removep":function(){return removep;}});const htmlSplitRegex=(()=>{const comments='!'+
'(?:'+
'-(?!->)'+
'[^\\-]*'+
')*'+
...
'|'+
'[^>]*>?'+
')'+')';return new RegExp(regex);})();function htmlSplit(input){const parts=[];let workingInput=input;let match;while(match=workingInput.match(htmlSplitRegex)){const index=match.index;parts.push(workingInput.slice(0,index));parts.push(match[0]);workingInput=workingInput.slice(index+match[0].length);}
...
return haystack;}
function autop(text){let br=arguments.length>1&&arguments[1]!==undefined?arguments[1]:true;const preTags=[];if(text.trim()===''){return '';}
